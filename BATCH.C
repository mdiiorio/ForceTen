...
while(i < que.files)
	...
	print("%-3s",ext);
	if(que.fstruct[i].filepoints==0)
		print("4] [2Free");
	if(que.fstruct[i].status!=0 && que.fstruct[i].filepoints!=0)
		{
...
for(i=0;i < que.files;i++)
	{
	nl();
	print("4Preparing 3%s...",que.fstruct[i].name);
...
print("4        컴컴컴");
nl();
print("4New FPS:  3%d",user.filepoints);
nl();
nl();
}
...
sprintf(s,"%s\\TEMPULE\\*.*",config.rootdir);
...
if(findfirst(s,&fi,0)==-1)
	...
	sprintf(s,"RD %s\TEMPULE",config.rootdir);
	system(s);
	return;
	}
...
if(ok)
	...
	system(k);
	sprintf(k,"DEL %s\\TEMPULE\\%s",config.rootdir,fstruct.name);
	system(k);
	print("Done!");
	nl();
	nl();
...
chdir(config.rootdir);
sprintf(k,"RD %s\\TEMPULE",config.rootdir);
system(k);
}
